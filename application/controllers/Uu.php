...
private $filename = "import_data"; // Kita tentukan nama filenya
	...
	*/
	...
	public function download_uu(){
		...
		// redirect('Uu');
		echo "<script>window.location='".site_url()."Uu'</script>";
	}
	...
	public function ubah_profil(){
		...
		// redirect('Uu/profil');
		echo "<script>window.location='".site_url()."Uu/profil'</script>";
	}
	...
	public function ubah_sandi(){
		...
		if($cek->num_rows() > 0){
			...
			echo "<script>window.location='".site_url()."Uu/ganti_sandi'</script>";
		}
		else{
	...
	public function ubah_email(){
		...
		if($cek->num_rows() > 0){
			...
			if(empty($cek2)){
				...
				echo "<script>window.location='".site_url()."Uu/ganti_email'</script>";
			}
			else{
			...
			if ($cek->num_rows() > 0) {
				...
				foreach ($cek->result() as $qad) {
					if($qad->status=='uu'){
						...
						// redirect('Uu/admin');
						echo "<script>window.location='".site_url()."Uu/admin'</script>";
					}
				}
			} else {
	...
	public function update_kategori(){
		...
		echo "<script>window.location='".site_url()."Uu/kategori'</script>";
	}
	...
	public function import(){
		...
		// redirect("Uu/form_import");
		echo "<script>window.location='".site_url()."Uu/form_input'</script>";
	}
		...
		if($nama!="")
		{
			echo '<ul>';
				...
				echo	'<li class="list-group-item" id="total">
							...
							Terdapat <b>'.$data['hasil_semua']->num_rows().'</b> hasil pencarian dengan kata "<b>'.$nama.'</b>"</a>
						</li>';
			echo '</ul>';
		}
		else
