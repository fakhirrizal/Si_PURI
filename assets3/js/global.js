...
function pdf_read(elem, ev){
    ...
    if( $("body").find("#"+id).length == 0 ){
        var content = `
            <iframe src='`+url+`'/>
        `;
        create_modal(id, 'Baca Lengkap', content);
    }
    $("body").find(".black-bg").fadeIn();
        ...
        <div class='black-bg' style='display:none'>
            <div class='modal' id='`+id+`'>
                <div class='modal-header'>
                    <h3>`+title+`</h3>
                    <label class='modal-close-button' onclick='close_modal(this)'><i class='ion-ios-close-outline'></i></label>
                </div>
